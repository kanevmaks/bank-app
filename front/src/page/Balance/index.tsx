...
import getHoursAndMinute from "../../utilits/GetHoursAndMinute";
        ...
        <div className="transactions-list">
          {transactions.reverse().map((item) => {
            ...
            if (item.type === "receive") {
              if (item.resEmail?.toLowerCase() === "stripe") {
                icon = "transaction__stripe";
                name = "Stripe";
              } else if (item.resEmail?.toLowerCase() === "coinbase") {
                icon = "transaction__coinbase";
                name = "Coinbase";
              } else {
                if (item.resEmail) {
                  icon = "transaction__user";
                  name = item.resEmail?.split("@")[0] as string;
